...
// Show donation promotion modal (always show on download)
function showDonationPromo() {
    ...
    document.getElementById('promoDonateBtn').addEventListener('click', async function() {
        ...
        try {
            // Redirect to donate page with amount
            window.location.href = `/donate?amount=${amount}&source=promo`;
        } catch (err) {
    ...
    // Remove auto-close timer - user must manually close
}
